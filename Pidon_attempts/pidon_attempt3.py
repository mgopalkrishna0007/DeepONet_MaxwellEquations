...
def generate_plane_wave_superposition(min_waves=1, max_waves=5):
    """
    Generate superposition of multiple plane waves E-field and its curl.
    Matches paper's equation: E = Σ E₀,i exp{-j k_i [...]}
    
    Args:
        min_waves: Minimum number of plane waves to superpose (default=1)
        max_waves: Maximum number of plane waves to superpose (default=5)
    
    Returns:
        E: Superposed E-field [3, Nx, Ny, Nz]
        curlE: Curl of superposed E-field [3, Nx, Ny, Nz]
    """
    # Randomly choose n between min_waves and max_waves (inclusive)
    num_waves = np.random.randint(min_waves, max_waves + 1)
